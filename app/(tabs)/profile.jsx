...
// Helper component for menu items (updated style)
...
export default function ProfileScreen() {
  ...
  const [daysLeft, setDaysLeft] = useState(7); // Default trial days
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      if (progressDoc) {
        ...
        // --- Trial Days Logic ---
        if (user.createdAt) {
          const creationDate = new Date(user.createdAt.seconds * 1000);
          const now = new Date();
          const diffTime = now - creationDate;
          const diffDays = Math.ceil(diffTime / (1000 * 60 * 60 * 24));
          const trialPeriod = 7; // Assuming a 7-day trial
  ...
  return (
    <SafeViewFromContext style={styles.container} edges={['top']}>
...
const styles = StyleSheet.create({
  ...
  subscriptionText: { color: '#E5E7EB', fontSize: 16, textAlign: 'center', marginBottom: 20 },
