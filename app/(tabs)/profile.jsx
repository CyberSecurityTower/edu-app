...
const SubscriptionCard = ({ subscription }) => {
  if (subscription && subscription.plan !== 'Trial' && subscription.status === 'active') {
    ...
    const formattedDate = renewalDate ? renewalDate.toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' }) : 'N/A';
    return (
      <View style={styles.subscriptionCard}>
        ...
        <Pressable style={{ marginTop: 15 }}><Text style={styles.manageLink}>Manage Subscription</Text></Pressable>
      </View>
    );
  }
  ...
  return (
    <View style={styles.subscriptionCard}>
      <Text style={styles.subscriptionText}>You have {daysLeft} {daysLeft === 1 ? 'day' : 'days'} left in free trial.</Text>
      <AnimatedGradientButton text="Upgrade Now" onPress={() => {}} buttonWidth={180} buttonHeight={45} fontSize={16} />
    </View>
  );
};
...
export default function ProfileScreen() {
  ...
  const handleLogout = () => {
    Alert.alert("Confirm Logout", "Are you sure you want to log out?", [
      ...
      { text: "Logout", style: "destructive", onPress: async () => { await signOut(auth); setUser(null); router.replace('/(auth)/'); }}
    ]);
  };
