...
export default function LessonViewScreen() {
  ...
  const processAndRespond = useCallback(async (userMessage, history) => {
    ...
    try {
      ...
      // ✨ [FINAL FIX] Explicitly find and replace the placeholder.
      setMessagesSafe(prev => {
        if (!Array.isArray(prev)) return [botResponse];
        const newMessages = [...prev];
        const typingId = seenToTypingRef.current[seenId];
        const typingIndex = typingId ? newMessages.findIndex(m => m.id === typingId) : -1;
        if (typingIndex !== -1) {
          newMessages[typingIndex] = botResponse;
          return newMessages.filter(m => m.id !== seenId);
        }
        const seenIndex = newMessages.findIndex(m => m.id === seenId);
        if (seenIndex !== -1) {
          newMessages[seenIndex] = botResponse;
          return newMessages;
        }
      ...
      if (error?.name === 'AbortError') {
        ...
        // ✨ [FINAL FIX] Explicitly replace placeholder with error message.
        setMessagesSafe(prev => {
           ...
           const typingIndex = typingId ? newMessages.findIndex(m => m.id === typingId) : -1;
           if (typingIndex !== -1) {
             newMessages[typingIndex] = errorMessage;
             return newMessages.filter(m => m.id !== seenId);
           }
           const seenIndex = newMessages.findIndex(m => m.id === seenId);
           if (seenIndex !== -1) {
             newMessages[seenIndex] = errorMessage;
             return newMessages;
           }
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top', 'left', 'right']}> 
      ...
      {isLoading ? (
        ...
        <View style={{ flex: 1 }}>
          {/* ✨ [FINAL FIX] Use conditional rendering to completely replace the background content */}
          {!isChatPanelVisible ? (
            <ScrollView contentContainerStyle={styles.contentContainer} scrollEventThrottle={400}>
              ...
              {/* The background robot is ONLY rendered here, ensuring it disappears completely */}
            ...
            // When chat is visible, show a static, non-interactive, and faded preview.
            // This is more performant and visually cleaner.
            <View style={{ flex: 1, padding: 20, opacity: 0.2 }} pointerEvents="none">
